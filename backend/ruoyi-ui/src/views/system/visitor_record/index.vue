    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:visitor_record:add']"
        >新增
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:visitor_record:edit']"
        >修改
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:visitor_record:remove']"
        >删除
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:visitor_record:export']"
        >导出
        </el-button>
      </el-col>
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listVisitorRecord(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      ...
      getVisitorRecord(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateVisitorRecord(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addVisitorRecord(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/visitor_record/export', {
        ...this.queryParams
