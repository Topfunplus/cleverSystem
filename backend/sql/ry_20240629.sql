...
(
    id                      INT AUTO_INCREMENT PRIMARY KEY,-- 访客ID，主键，唯一标识每个访客
    NAME VARCHAR(100) NOT NULL,-- 姓名，访客的真实姓名
    phone VARCHAR(20) NOT NULL,-- 手机号，访客的联系电话
    id_card VARCHAR(20) NOT NULL,-- 身份证号，访客的身份证号码，用于身份识别
    visitor_unit VARCHAR(100),-- 访客单位，访客所在单位名称
    visit_reason TEXT,-- 到访理由，访客到工厂的目的或理由
    id_card_front_image BLOB,-- 身份证正面照片，存储身份证正面的图像（二进制数据）
    id_card_back_image BLOB,-- 身份证反面照片，存储身份证背面的图像（二进制数据）
    face_image BLOB,-- 人脸照片，访客的脸部照片，用于闸机识别（二进制数据）
    is_car BOOLEAN DEFAULT 0,-- 是否开车，0表示未开车，1表示开车，默认未开车
    car_plate VARCHAR(20),-- 车牌号，访客的车牌号码，开车时需要填写
    visit_time DATE,-- 访客时间，访客计划进入的日期（精确到天）
    visit_duration INT,-- 访问时长，访客预定的访问时间，单位：分钟
    request_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,-- 申请时间，访客提交申请的时间（自动生成）
    STATUS ENUM ( '待审核', '已批准', '已拒绝' ) DEFAULT '待审核',-- 审核状态，表示当前申请的审批状态（待审核、已批准、已拒绝）
    first_approval_time     TIMESTAMP,-- 一级审核时间，第一次审核的时间，表示审核员对申请的时间戳
    first_approval_opinion  TEXT,-- 一级审核意见，审核员对申请的意见或说明
    second_approval_time    TIMESTAMP,-- 二级审核时间，第二次审核的时间，表示二级审核员的审核时间
    second_approval_opinion TEXT -- 二级审核意见，二级审核员的审核意见或说明

);-- 给 `visitor_info` 表中的字段添加注释
ALTER TABLE visitor_info
    COMMENT = '访客信息表，记录访客的基本信息以及审核状态';
ALTER TABLE visitor_info
    MODIFY id COMMENT '访客ID，主键，唯一标识每个访客';
ALTER TABLE visitor_info
    MODIFY NAME COMMENT '访客姓名，实际到访人的姓名';
ALTER TABLE visitor_info
    MODIFY phone COMMENT '访客的联系电话，用于联系访客';
ALTER TABLE visitor_info
    MODIFY id_card COMMENT '访客身份证号，用于唯一标识访客身份';
ALTER TABLE visitor_info
    MODIFY visitor_unit COMMENT '访客所在单位名称';
ALTER TABLE visitor_info
    MODIFY visit_reason COMMENT '访客到工厂的目的或理由';
ALTER TABLE visitor_info
    MODIFY id_card_front_image COMMENT '访客身份证正面照片，存储为二进制数据';
ALTER TABLE visitor_info
    MODIFY id_card_back_image COMMENT '访客身份证背面照片，存储为二进制数据';
ALTER TABLE visitor_info
    MODIFY face_image COMMENT '访客的面部照片，用于闸机识别';
ALTER TABLE visitor_info
    MODIFY is_car COMMENT '是否开车，0为未开车，1为开车';
ALTER TABLE visitor_info
    MODIFY car_plate COMMENT '访客的车牌号，仅在开车时填写';
ALTER TABLE visitor_info
    MODIFY visit_time COMMENT '访客的到访日期，精确到天';
ALTER TABLE visitor_info
    MODIFY visit_duration COMMENT '访客的访问时长，单位为分钟';
ALTER TABLE visitor_info
    MODIFY request_time COMMENT '申请时间，记录访客申请的时间';
ALTER TABLE visitor_info
    MODIFY STATUS COMMENT '审核状态，表示申请的审批状态：待审核、已批准、已拒绝';
ALTER TABLE visitor_info
    MODIFY first_approval_time COMMENT '一级审核时间，记录第一次审核的时间';
ALTER TABLE visitor_info
    MODIFY first_approval_opinion COMMENT '一级审核员的审核意见';
ALTER TABLE visitor_info
    MODIFY second_approval_time COMMENT '二级审核时间，记录二级审核的时间';
ALTER TABLE visitor_info
    MODIFY second_approval_opinion COMMENT '二级审核员的审核意见';-- 创建审核记录表
CREATE TABLE audit_record
(
    id               INT AUTO_INCREMENT PRIMARY KEY,-- 审核记录ID，主键，唯一标识每条审核记录
    visitor_id       INT                     NOT NULL,-- 访客ID，外键，关联到访客信息表，标识审核记录属于哪个访客
    audit_level ENUM ( '一级', '二级' ) NOT NULL,-- 审核级别，表示当前审核的级别，一级或二级
    auditor_id       INT                     NOT NULL,-- 审核人ID，外键，关联到用户表，表示审核人的ID
    approval_status  ENUM ( '同意', '拒绝' ) NOT NULL,-- 审批状态，表示该级审核员对申请的审批结果（同意或拒绝）
    approval_time    TIMESTAMP DEFAULT CURRENT_TIMESTAMP,-- 审批时间，表示当前审核的时间（自动生成）
    approval_opinion TEXT,-- 审批意见，审核员对该申请的具体审批意见
    FOREIGN KEY (visitor_id) REFERENCES visitor_info (id),-- 外键，关联到访客信息表
    FOREIGN KEY (auditor_id) REFERENCES users (id) -- 外键，关联到用户表的审核员

);-- 给 `audit_record` 表中的字段添加注释
ALTER TABLE audit_record
    COMMENT = '审核记录表，记录每一次对访客的审核记录';
ALTER TABLE audit_record
    MODIFY id COMMENT '审核记录ID，主键，唯一标识每条审核记录';
ALTER TABLE audit_record
    MODIFY visitor_id COMMENT '访客ID，外键，关联到访客信息表';
ALTER TABLE audit_record
    MODIFY audit_level COMMENT '审核级别，表示当前审核是一级审核还是二级审核';
ALTER TABLE audit_record
    MODIFY auditor_id COMMENT '审核人ID，外键，表示该条审核记录由哪个用户审核';
ALTER TABLE audit_record
    MODIFY approval_status COMMENT '审批状态，表示该级审核员对申请的审批结果（同意或拒绝）';
ALTER TABLE audit_record
    MODIFY approval_time COMMENT '审批时间，记录审核员审批的时间';
ALTER TABLE audit_record
    MODIFY approval_opinion COMMENT '审批意见，审核员对该申请的具体意见';-- 创建二维码生成记录表
CREATE TABLE qr_code_record
(
    id            INT AUTO_INCREMENT PRIMARY KEY,-- 二维码ID，主键，唯一标识每条二维码记录
    visitor_id    INT NOT NULL,-- 访客ID，外键，关联到访客信息表，表示该二维码对应的访客
    qr_code TEXT NOT NULL,-- 二维码内容，存储二维码的字符串，包含访客的基本信息（如姓名、身份证号等）
    creation_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,-- 创建时间，二维码生成的时间（自动生成）
    FOREIGN KEY (visitor_id) REFERENCES visitor_info (id) -- 外键，关联到访客信息表

);-- 给 `qr_code_record` 表中的字段添加注释
ALTER TABLE qr_code_record
    COMMENT = '二维码生成记录表，记录每条生成的二维码及其相关信息';
ALTER TABLE qr_code_record
    MODIFY id COMMENT '二维码ID，主键，唯一标识每条二维码记录';
ALTER TABLE qr_code_record
    MODIFY visitor_id COMMENT '访客ID，外键，关联到访客信息表，表示二维码对应的访客';
ALTER TABLE qr_code_record
    MODIFY qr_code COMMENT '二维码内容，包含访客的基本信息，用于闸机识别';
ALTER TABLE qr_code_record
    MODIFY creation_time COMMENT '二维码创建时间，记录二维码生成的时间';-- 创建用户表
CREATE TABLE users
(
    id          INT AUTO_INCREMENT PRIMARY KEY,-- 用户ID，主键，唯一标识每个用户
    username    VARCHAR(50) NOT NULL,-- 用户名，用户的登录用户名
    PASSWORD VARCHAR(255) NOT NULL,-- 密码，用户的加密密码
    role ENUM ( '管理员', '审核员' ) NOT NULL,-- 角色，标识该用户是管理员还是审核员
    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP -- 创建时间，表示用户账号的创建时间（自动生成）

);-- 给 `users` 表中的字段添加注释
ALTER TABLE users
    COMMENT = '用户表，记录系统中的用户信息，管理员与审核员';
ALTER TABLE users
    MODIFY id COMMENT '用户ID，主键，唯一标识每个用户';
ALTER TABLE users
    MODIFY username COMMENT '用户名，用户用于登录的用户名';
ALTER TABLE users
    MODIFY PASSWORD COMMENT '密码，用户的加密密码';
ALTER TABLE users
    MODIFY role COMMENT '角色，表示该用户的角色（管理员或审核员）';
ALTER TABLE users
    MODIFY create_time COMMENT '创建时间，记录用户账号创建的时间';-- 创建访问记录表
CREATE TABLE visit_record
(
    id         INT AUTO_INCREMENT PRIMARY KEY,-- 访问记录ID，主键，唯一标识每条进出记录
    visitor_id INT NOT NULL,-- 访客ID，外键，关联到访客信息表，标识该访问记录属于哪个访客
    entry_time TIMESTAMP,-- 进入时间，访客进入工厂的具体时间（时间戳）
    exit_time  TIMESTAMP,-- 离开时间，访客离开工厂的具体时间（时间戳）
    is_reentry BOOLEAN DEFAULT 0,-- 是否是重新进入，0表示首次进入，1表示重新进入，默认首次进入
    FOREIGN KEY (visitor_id) REFERENCES visitor_info (id) -- 外键，关联到访客信息表

);-- 给 `visit_record` 表中的字段添加注释
ALTER TABLE visit_record
    COMMENT = '访问记录表，记录访客的进出情况';
ALTER TABLE visit_record
    MODIFY id COMMENT '访问记录ID，主键，唯一标识每条进出记录';
ALTER TABLE visit_record
    MODIFY visitor_id COMMENT '访客ID，外键，关联到访客信息表';
ALTER TABLE visit_record
    MODIFY entry_time COMMENT '访客进入工厂的时间，精确到时间戳';
ALTER TABLE visit_record
    MODIFY exit_time COMMENT '访客离开工厂的时间，精确到时间戳';
ALTER TABLE visit_record
    MODIFY is_reentry COMMENT '是否为重新进入，0表示首次进入，1表示重新进入';